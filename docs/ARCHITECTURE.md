...
The main Collector batches all signals. Logs and traces use separate bounded export queues backed by `file_storage`; metrics use a Prometheus exporter and scrape path. The fault relay returns 503 for a bounded period, then forwards the original OTLP payload to the configured backend. It cannot choose an arbitrary target from a user request.

The experiment runner polls Jaeger and Loki for actual delivery. It saves request IDs, trace IDs, latency measurements, queue samples, assertions, and errors to SQL. Missing measurements remain unavailable. Workload delivery scenarios require the expected evidence; isolated expected-loss scenarios pass only when their loss and recovery-canary assertions hold. Metric comparisons are scoped to the three running service instance IDs so a restarted process does not contaminate a new run's cardinality comparison.
...
The [Gradio reliability workbench](TELEMETRY_LAB.md) adds isolated persistent/volatile crash, retry-expiry, and queue-saturation experiments, durable progress records, cancellation, and portable reports. Its direct-OTLP collectors omit batching to isolate the queue boundary.
...
| UI | Gradio, port 7860; optional catalog Streamlit, port 8501 | Gradio, port 7860 |
...
The database tables are created for a new lab on startup. No schema migration framework is included. Authentication is an optional shared API bearer token; it is not user authorization. Gradio and Streamlit send API requests on their servers, so configuring an API token does not authenticate people who can reach either UI. Keep the demo local, or place every exposed surface behind an authenticated TLS proxy before sharing a hosted instance.
